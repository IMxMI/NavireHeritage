...
namespace NavireHeritage.ClassesTechniques
{
    class Test
    {
		...
		public static void AfficheAttendus(Port port)
		{
			...
			foreach (Navire navire in port.NavireArrives.Values)
			{
				if (navire is Cargo cargo)
				{
					Console.WriteLine($"IMO : {cargo.Imo}\t{cargo.Nom} : {cargo.GetType().Name}");
				}
				else if (navire is Tanker tanker)
				{
					Console.WriteLine($"IMO : {tanker.Imo}\t{tanker.Nom} : {tanker.GetType().Name}");
				}
				else if (navire is Croisiere croisiere)
				{
					Console.WriteLine($"IMO : {croisiere.Imo}\t{croisiere.Nom} : {croisiere.GetType().Name}");
				}
				else
				{
					Console.WriteLine($"Le bateau {navire.Imo} est d'in autre type");
				}
			}
